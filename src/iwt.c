...
double iwt_alpha_IWT() { return 1; }

double iwt_beta_IWT() { return 1; }

int iwt_classify(double Re, double Im)
{
	double abs_I = sqrt(Re * Re + Im * Im + 1e-30);

	if (fabs(abs_I - 0.01) < 0.001)
		return 0; // Vakuum
	if (fabs(abs_I - 0.25) < 0.001)
		return 1; // Elektron
	if (fabs(abs_I - 1.0) < 0.001)
		return 2; // Proton
	if (fabs(abs_I - 0.05) < 0.001)
		return 3; // u-Quark
	if (fabs(abs_I - 0.06) < 0.001)
		return 4; // d-Quark
	return -1;	  // sonst
}

void iwt_compute_spectrum(const double *I_real, const double *I_imag, size_t N,
						  struct iwt_spectrum *spec)
{
	spec->count_vacuum = 0;
...
{
	size_t total = spec->count_vacuum + spec->count_electron +
				   spec->count_proton + spec->count_u_quark +
				   spec->count_d_quark + spec->count_other;

	printf("=== Teilchenspektrum ===\n");
	printf("Vakuum   (0.01): %zu (%.2f%%)\n", spec->count_vacuum,
		   100.0 * spec->count_vacuum / total);
	printf("Elektron (0.25): %zu (%.2f%%)\n", spec->count_electron,
		   100.0 * spec->count_electron / total);
	printf("Proton   (1.0) : %zu (%.2f%%)\n", spec->count_proton,
		   100.0 * spec->count_proton / total);
	printf("u-Quark  (0.05): %zu (%.2f%%)\n", spec->count_u_quark,
		   100.0 * spec->count_u_quark / total);
	printf("d-Quark  (0.06): %zu (%.2f%%)\n", spec->count_d_quark,
		   100.0 * spec->count_d_quark / total);
	printf("Sonstige       : %zu (%.2f%%)\n", spec->count_other,
		   100.0 * spec->count_other / total);
	printf("Gesamt: %zu Knoten\n", total);
}

private
char *iwt_get_exe_path(void)
{
...
bool iwt_save_state(const iwt_runtime_t rt, const iwt_config_t cfg,
					const char *filename)
{
	...
	if (fwrite(rt->I_real, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fwrite(rt->I_imag, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fwrite(rt->I_prev_real, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fwrite(rt->I_prev_imag, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fwrite(rt->I_phase, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fwrite(rt->I_phase_prev, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	...
	if (fwrite(rt->xi_real, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fwrite(rt->xi_imag, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
...
bool iwt_load_state(const iwt_runtime_t rt, const iwt_config_t cfg,
					const char *filename)
{
	char *exe_dir = iwt_get_exe_path();
	if (!exe_dir)
	...
	// === IWT-Kernfelder (komplex) ===
	size_t n = cfg->N * sizeof(double);
	if (fread(rt->I_real, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fread(rt->I_imag, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fread(rt->I_prev_real, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fread(rt->I_prev_imag, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fread(rt->I_phase, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fread(rt->I_phase_prev, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	...
	if (fread(rt->xi_real, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
	if (fread(rt->xi_imag, 1, n, f) != n)
	{
		fclose(f);
		return false;
	}
...
private
double iwt_power_iteration(double *A, double *v, size_t n,
						   int max_iter)
{
...
bool iwt_mds_compute(const iwt_runtime_t rt, const iwt_config_t cfg,
	iwt_mds_t mds)
{
	size_t N = cfg->N;
	if (N < 2)
...
void iwt_mds_save_pgm(const iwt_mds_t mds, const double *I, size_t N,
					  const char *filename)
{
	if (!mds->coords || !I || N != mds->N)
		return;
...
private
void iwt_print_int(uint32_t *x, uint32_t y, const char *label, int value)
{
...
private
void iwt_print_double(uint32_t *x, uint32_t y, const char *label,
					  double value)
{
	string_set_cursor_position(*x, y);
	char buffer[256] = {0};
...
void iwt_print_status(const iwt_runtime_t rt, const iwt_config_t cfg, int iter,
					  ...
					  double deviation, double sum_I_sq,
					  double info_deviation)
{
	...
	iwt_print_double(&col, row, "phase[101]",
					 rt->I_phase[101] / iwt_pi() * 180.0);
	iwt_print_double(&col, row, "phase[102]",
					 rt->I_phase[102] / iwt_pi() * 180.0);
	iwt_print_double(&col, row, "phase[103]",
					 rt->I_phase[103] / iwt_pi() * 180.0);
	iwt_print_double(&col, row, "phase[104]",
					 rt->I_phase[104] / iwt_pi() * 180.0);
...
void iwt_save_heatmap(const double *data, size_t N, const char *filename,
					  const char *type)
{
	// Finde Min/Max
	double min_val = 1e30;
	...
	// MDS-Koordinaten werden benötigt – wir verwenden eine einfache
	// 1D-zu-2D-Abbildung (später mit MDS-Koordinaten ersetzen)
